...
var jwt = require("jsonwebtoken");
 ...
 export async function PUT(req) {
    ...
    try{
       const {jwtToken,id,fName,lName,phone,city,address,postal,country,admin} = await req.json() 
    mongoose.connect(process.env.MONGO_URI)
    if(jwtToken){
     const jwtverify = jwt.verify(jwtToken,process.env.JWT_SECRET_KEY) 
     const updateUserProfile = await user.findOneAndUpdate({_id:jwtverify.id},{
   ...
   const updateUserProfile = await user.findByIdAndUpdate({_id:id},{
    firstName:fName,lastName:lName,number:phone,city:city,address:address,postalCode:postal,country:country,admin:admin
